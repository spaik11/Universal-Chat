...
const { translateMsg } = require('../socket/helperFunc/translate');
...
router.get('/', async (req, res, next) => {
  try {
    ...
    console.log('server language GET ', language);
    const messages = await Message.findAll();

    messages.map(
      async ({ content }) => (content = await translateMsg(content, language))
    );
